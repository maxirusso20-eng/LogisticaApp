...
export default function MapaScreen() {
  ...
  // ── 2. Realtime: escuchar cambios de posición de todos los choferes
  useEffect(() => {
    fetchVehiculos();

    const channel = supabase
      .channel('mapa-gps-sync')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'Choferes' }, fetchVehiculos)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'Choferes' }, fetchVehiculos)
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'Choferes' }, fetchVehiculos)
      .subscribe();
  ...
  useEffect(() => {
    ...
    const iniciarRastreo = async () => {
      ...
      if (status !== 'granted') {
        setErrorPermiso('Permiso de ubicación denegado. Activalo en Configuración para compartir tu posición.');
        return;
      }
      ...
      watcherRef.current = await Location.watchPositionAsync(
        {
          ...
          distanceInterval: 15,  // actualiza cada 15 metros
  ...
  // ── Centrar el mapa en mi ubicación actual
  const centrarEnMi = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <MapView
        ...
        mapType="standard"
      >
        {/* Marcadores de cada chofer con GPS activo */}
        {vehiculosConGPS.map(v => (
          <Marker
            key={v.id}
            coordinate={{
              latitude:  Number(v.latitud),
              longitude: Number(v.longitud),
            }}
            title={v.nombre}
      ...
      {miUbicacion && (
        <TouchableOpacity style={styles.btnMiPosicion} onPress={centrarEnMi} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  // Barra de estadísticas superpuesta en la parte superior
  statsBar: {
  ...
  errorBanner: {
    position: 'absolute', bottom: 100, left: 16, right: 70,
  ...
  // Panel de detalle del chofer seleccionado
  detailPanel: {
